...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "not a valid address to transfer ownership.");
...
contract Pausable is Ownable {
    ...
    function setNotPaused() external onlyOwner whenPaused {
        _paused = false;
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, "must be a different address from token owner.");
        require(msg.sender == owner || isApprovedForAll(owner, to), "must be token owner or approved for all to approve.");
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, "approver and sender must be different.");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "must be approved or token owner.");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "check on ERC721Received failed.");
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        require(from == _tokenOwner[tokenId], "from must be token owner.");
